...
void setupBrew() {
    ...
    brewStartMs = substateEntryMs = millis();
}
...
void updateBrewStateMachine() {
    ...
    if (ms != STATE_ERROR) {
        ...
        else if (swSteam && swCoffee) { newMs = STATE_ERROR; err = ERR_BOTH_SWITCHES; }
    ...
    float target = (newMs == STATE_STEAM) ? p.steamTargetTemp : p.coffeeTargetTemp;
    ...
    bool wasActive = (ms    == STATE_COFFEE || ms    == STATE_STEAM);
    bool nowActive = (newMs == STATE_COFFEE || newMs == STATE_STEAM);
